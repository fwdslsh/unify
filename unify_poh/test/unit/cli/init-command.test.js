...
describe('InitCommand', () => {
  ...
  describe('extractTemplate', () => {
    test('should_extract_zip_archive_when_valid_zip_provided', async () => {
      // This would test the actual zip extraction logic
      // For now, we'll test the interface with valid-sized zip data
      const zipBuffer = new ArrayBuffer(1024); // Mock zip data with valid size
      const result = await initCommand.extractTemplate(zipBuffer, tempDir);
    ...
    test('should_preserve_directory_structure_when_extracting', async () => {
      ...
      const result = await initCommand.extractTemplate(new ArrayBuffer(1024), tempDir);
