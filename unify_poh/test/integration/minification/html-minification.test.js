...
<html>
  ...
  <body>
    ...
    try {
      await cli.run([
        'build',
        '--source', tempProject.getPath('src'),
        '--output', tempProject.getPath('dist')
      ]);
    } catch (error) {
      // Expected due to mocked process.exit
    }
    ...
    const outputHtml = await tempProject.readFile('dist/index.html');
...
<html>
  ...
  <body>
    ...
    const outputHtml = await tempProject.readFile('dist/code.html');
