...
<body>
  ...
  test('should_handle_processing_errors_gracefully_with_security_scanning', async () => {
    ...
    // Should succeed with fallback processing and still scan for security issues
    expect(result.success).toBe(true);
    expect(result.html).toContain('Should still be scanned'); // Fallback processing preserves content
    // Security scanning should detect the onclick handler
    expect(result.securityWarnings).toHaveLength(1);
