...
- **Subpath hosting.** GitHub Pages project sites serve from `username.github.io/repo-name/`, where root-relative links would break. `--base-url /repo-name/` prefixes every root-relative URL in the built HTML — `href`, `src`, `srcset`, `poster`, and the URL values in `og:`/`twitter:` metas; source files stay rooted at `/`, so local preview keeps working. `--base-url` also accepts a full URL (`https://example.com/repo/`): og:/twitter:/canonical values, which crawlers require to be absolute, are then absolutized against the whole base — origin **and** subpath — so an og:image lands at `https://example.com/repo/assets/x.jpg`, exactly where the file is served.
