...
- **Pretty URLs move pages, never assets.** Every reference in a moved page — `href`, `src`, `srcset`, `poster`, the URL-valued `og:`/`twitter:` meta content, and a `<meta http-equiv="refresh">` URL — is rewritten to keep pointing at the same target, so `![diagram](diagram.png)` beside a Markdown page keeps working and a page-targeting `og:url`/`og:image` comes out in the same directory spelling as the matching `href`. A root `404.html` is never moved: hosts look for it at that exact path.
...
- **No recursive DOM serialization, and no post-build mutation.** The catalog (§6.5.2) is a bounded structural projection — root attributes, head data, and a flat heading list — never a JSON re-encoding of a page's whole DOM, and it carries no body paragraphs, code blocks, lists, or other raw article markup; full text lives only in the separate, opt-in search corpus, as normalized text rather than markup. Nothing in the production-and-discovery layer reaches back into an already-emitted page and edits it after the fact — a future post-build step, if one is ever added, may only generate additional artifacts from the final manifest, never rewrite HTML the build already published.
- **unify is not advertised primarily for blogs or documentation sites.** Both work — the blog template plus its generator is a real workflow, and Markdown pages get heading anchors (§3.5) — but publication sites want feeds and documentation sites want generated navigation or search. Launch positioning remains where unify is unambiguously strongest: brochure, portfolio, campaign, and project sites; existing hand-written HTML adopting shared chrome; SSI migration. That positioning broadens only after the feed, catalog/corpus, audit, and generator workflows are proven without turning composition into a template or component system (§6).
...
### 6.2 Foundation: the final document model

After composition and URL rewriting, unify should derive one internal document for every composed page — the same set the reference check validates and the publish step writes, no more and no fewer. Each document is a thin envelope around a bounded, publishable **snapshot** of the page's own emitted HTML — root attributes, head data (title, meta, links), body attributes, and a flat heading sequence, read from the exact bytes a browser or a crawler will actually receive — plus a heavier, build-only **analysis** (visible main text, every `id`, structured-data blocks, the internal link graph, and the like) that stays private to the build and is never dumped into an output file wholesale. Extraction reads the emitted document rather than frontmatter or source markup, so HTML and Markdown remain equal citizens and layout-provided metadata is represented exactly as it ships.

A page with conflicting declarations — two titles, two canonicals — keeps the first in document order for each field and reports each conflicted field as one located finding; the computation happens once. That is the manifest's own law, and it governs every feature built on it: **there is one shared interpretation of a page, not a separate reading for every consumer that needs one.** A shared selector layer is what enforces it — sitemap generation, canonical completion, robots consistency, structured-data checks, feeds, the catalog and search corpus (§6.5.2), orphan detection, and machine-readable reports all interpret the same snapshot and analysis through the same functions rather than re-parsing a page or deciding a value twice; a fact is computed on demand and never written back onto the envelope as a new stored field, so two consumers cannot silently disagree about what a page's title, canonical, or declared type is.

The manifest remains an **implementation boundary**, not a new file format authors must learn: no command writes it as such, and no authoring rule mentions it. What authors interact with instead are its two public projections (§6.5.2) and the evaluation commands built on top of it.
...
9. **Counter-prior diagnostics.** Frontmatter that authors and coding agents routinely expect other generators to interpret must not silently imply behavior unify does not have. `draft`, `permalink`, and `slug` are located problems naming the unify mechanism: prefix the source name with `_` to hold a page back; rename or move the source file to choose its output path; and use `--pretty-urls` only to apply unify's site-wide `.html`-to-directory rewrite. `tags` and `categories` are not addressed to the build at all: they synthesize to ordinary `<meta>` tags exactly like any other frontmatter key, unify reports nothing about either one, and they are inert by design — meaningful only to a consumer that chooses to interpret them, a catalog consumer (§6.5.2) included. These diagnostics exist to prevent confident cross-generator assumptions from publishing or addressing the wrong page, not to reserve ordinary metadata names without cause, and `tags`/`categories` never carried that failure mode. Existing diagnostics for bare layout names, path-only `--base-url` values, hand-written pretty URLs, and non-empty includes remain mandatory.
...
2. **`--generate <path>` instead of `--run <cmd>`.** Let `build`, `watch`, `dev`, and `audit` run one author-owned JavaScript generator from the source tree before scanning pages. The path is explicit and saved like any other flag; it is not an arbitrary shell command. The standalone binary supplies the runtime, sets the working directory to the source root, and runs the script as a subprocess with three positional arguments (`process.argv[2]`, `[3]`, `[4]`): the absolute source path, a temporary generated-source directory, and the path to a small, versioned, read-only JSON snapshot of the build's own effective settings — source/output paths, the site's public address and pretty-URL/canonical settings, and where the catalog and search corpus will land if either was requested, but never a page manifest, since none exists yet at this point in the build. A generator needs no imported unify API and may read at most those three arguments. Only files written to the generated directory join the scan as an overlay, so the supported workflow does not mutate `src/`, audit remains read-only, generated paths participate in collision checks, and watch mode cannot trigger on its own temporary outputs. Generator failures use unify's located diagnostics, and generated pages then follow the ordinary build contract. The filesystem remains the extension interface—this only removes the second runtime and second watcher from the common workflow.
...
2. **Catalog and search corpus.** Two independent, optional JSON projections of §6.2's manifest, both under `assets/unify/` — not a second extractor or vocabulary, since each is built by reading the identical snapshot and analysis every other consumer reads. `--catalog` writes `assets/unify/catalog.json`: a compact, HTML-shaped structural projection of every public page — its root attributes, head data (title, meta, link, base), body attributes, and a flat heading sequence — for browse, filter, TOC, and metadata-driven UI. It carries no body text and no JSON-LD script bodies; those stay bounded by design, not by omission. `--search-corpus` writes `assets/unify/search-corpus.json`: a minimal full-text projection — a page's path plus its normalized visible text, nothing else — for a small client-side search library or external indexer to index however it chooses. Neither flag implies the other. The two files share exactly one join key, `path`, so a consumer builds one lookup from the catalog and resolves every search hit against it rather than duplicating title, description, or headings into the corpus. Both are versioned (`schemaVersion`), an authored file at either path suppresses generation and ships as written, and both are deterministic across repeated builds of one tree. unify does not ship a search runtime.
3. **Opt-in network audit and reports.** `unify audit --external` checks external links without making normal builds network-dependent. `unify audit --format json` exposes findings plus one page entry per document — its source path, whether it came from a generator, its output path, and the page's own document snapshot (§6.2) serialized whole rather than a hand-picked subset of fields — for CI and other tools, without creating a plugin API or a second reading of a page. The report has a `schemaVersion`; each finding has a stable machine identifier, severity, source/output locations where available, public URL, evidence, suggested fix, and stable fingerprint. Human output remains plain language and never requires memorizing those identifiers. A SARIF serializer is optional only if it is a mechanical view of the same findings rather than another analysis path.
...
- At least one external open-source project adopts unify for its website — the first proof of the "more peaceful internet" ambition. Publication and documentation sites become headline use cases only after the feed, catalog/corpus, audit, and generator workflows make them comparably boring to maintain.
