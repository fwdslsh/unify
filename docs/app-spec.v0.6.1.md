...
- **`data-for`** - Targets content to elements with matching IDs within imported fragments
  - Used on elements inside `data-import` containers to specify target elements by ID
  - Value must match `id` attribute exactly (case-sensitive) of an element in the imported fragment
  - Elements without `data-for` replace the entire body content of the imported fragment
  - The target element's `id` and `class` attributes are preserved; all other attributes are merged with source winning conflicts
  - Removed from final output
...
- **`<body>`** - Document body with class merging and content injection
  ...
  - Body content is injected into layout elements during composition
...
- **Attribute Removal**: `data-import`, `data-for`, and `data-layer` attributes are stripped from final output
- **Path Resolution**: All `data-import` paths support both absolute/relative file paths and short name resolution
- **Nested Composition**: ID targeting and imports work recursively - imported fragments can import other fragments
- **Fallback Content**: Default content in the imported fragment is preserved when no matching `data-for` is provided
...
- **Cascading Imports** (`data-import`/ID targeting / `data-for`): Unified mechanism for fragments composition
...
2. **Check if transformation applies**:
   ...
   - Must end with `.html`/`.htm` extension
...
- **Implicit Assets Copy:** The `assets/**` directory is implicitly copied to output unless explicitly excluded (mirrors Astro/Vite "public" directory behavior)
...
##### ID-based Targeting in Includes

The elements with `data-import` support ID-based content targeting, allowing you to pass content to specific elements within the included component:

```html
<!-- Component: _includes/nav.html -->
<nav class="navbar">
  <a href="/" id="brand" class="brand-link">Default Brand</a>
  <ul id="nav-menu" class="nav-list">
    <li><a href="/">Home</a></li>
  </ul>
  <div id="nav-actions" class="actions">
...
<body>
  <div data-import="/_includes/nav.html">
    <a href="/" data-for="brand" data-logo="/logo.png">MyBrand</a>

    <ul data-for="nav-menu">
      <li><a href="/docs/">Docs</a></li>
      <li><a href="/blog/">Blog</a></li>
    </ul>

    <a href="/start/" data-for="nav-actions" class="btn">Get Started</a>
  </div>
</body>

<!-- Output -->
<nav class="navbar">
  <a href="/" id="brand" class="brand-link" data-logo="/logo.png">MyBrand</a>
  <ul id="nav-menu" class="nav-list">
    <li><a href="/docs/">Docs</a></li>
    <li><a href="/blog/">Blog</a></li>
  </ul>
  <a href="/start/" id="nav-actions" class="actions btn">Get Started</a>
</nav>
```

**ID-based Targeting Rules:**

1. **ID Matching**: Elements with `data-for="id"` are matched to corresponding elements with `id="id"` in the imported fragment
2. **Content Replacement**: The content of the target element is replaced by the content of the element with matching `data-for` attribute
3. **Attribute Merging**: 
   - Target element's `id` and `class` attributes are preserved
   - All other attributes from the source element are merged with the target element
   - Source element attributes override target element attributes in conflicts
   - Classes are merged (not overwritten)
4. **Element Type Override**: The source element's tag name replaces the target element's tag name
5. **Attribute Removal**: `data-import` and `data-for` attributes are removed from final output
6. **Fallback Content**: If no content with matching `data-for` is provided, the original content inside the target element is preserved
7. **Multiple Targets**: Multiple elements can target different IDs in the imported fragment
8. **Nested Includes**: ID-based targeting works with nested includes - targets are resolved at each level

**Important Notes:**

- ID-based targeting only works with elements, not with Apache SSI includes
- The `data-for` attribute value must match the target `id` attribute value exactly (case-sensitive)
- Elements without `data-for` attributes inside `data-import` elements replace the entire body content of the imported fragment
...
- Apache SSI includes do not support ID-based targeting
...
<div data-import="/layouts/blog.html">
  <h1>Article Title</h1>
  <p>Article content goes into the default content area...</p>
</div>
```

**Markdown Support**: When the imported file has a `.md` extension, it is processed through the markdown processor and the resulting HTML is included. Frontmatter is processed but not included in the output. Markdown do not support ID targeting.
...
#### ID Targeting and Content Injection

Layouts define targetable elements using standard `id` attributes:
...
<html>
  ...
  <body>
    <header>
      <h1 id="page-header" class="header-title">Default Header</h1>
    </header>
    <main id="main-content" class="content">
      <!-- Default content area for main content -->
    </main>
    <aside>
      <div id="sidebar" class="sidebar-content">Default Sidebar</div>
    </aside>
  </body>
</html>
```

Pages provide content for target elements using `data-for="id"`:

```html
<div data-import="/layouts/blog.html">
  <!-- This content goes to the main-content element -->
  <article data-for="main-content">
    <h1>Article Title</h1>
    <p>Article content...</p>
  </article>

  <!-- Target specific elements by ID -->
  <h1 data-for="page-header" class="custom-header">Custom Blog Header</h1>

  <nav data-for="sidebar" class="article-nav">
...
3. **ID Resolution**: `data-for="id"` elements target elements with `id="id"` in the imported fragment
4. **Default Content**: Content not using `data-for` replaces the entire body content of the imported fragment
5. **Fallback Content**: Content inside target elements serves as fallback if no matching `data-for` is provided
...
- A `data-for="id"` targets an element ID not present in the imported fragment
...
```
Warning: Element data-for="sidebar" has no matching id="sidebar" in imported fragment
```

**Cause**: Page provides content for an ID that doesn't exist in the layout.
**Solution**: Add the target ID to the layout or remove the `data-for` from the page.
...
- The converted HTML body replaces the entire body content of the importing element.
...
- Only children of this scope may target elements inside the imported fragment
...
<html>
  ...
  <body>
    <header>
      <h1 id="site-header" class="header">Default</h1>
    </header>
    <main id="main-content">
      <!-- default content area -->
    </main>
    <footer><span id="footer-text">© Site</span></footer>
  </body>
</html>
```

- **Target elements**: Elements with `id` attributes for specific content targeting
- **Default content**: Content without `data-for` replaces the entire body content
...
<body data-import="/layouts/base.html">
  <p>Body content (replaces entire body when no data-for specified)</p>
  <h1 data-for="site-header" class="welcome">Welcome!</h1>
</body>
```

- `data-for` specifies the target element ID
- Content of the targeting element replaces the target element's content
- Target element's `id` and `class` are preserved, other attributes are merged
- Target element's tag name replaces the original element's tag name
...
- Target elements are updated by matching `data-for` elements. Exception: `<head>` contributions are merged globally
- `html` and `body` attributes are merged globally with body class merging (merge, don't overwrite)
- For content without `data-for`: replaces entire body content of imported fragment
- Unmatched `data-for`: no-op with dev warning
- **Warnings, not surprises**: Unknown `data-for` or missing target IDs warn in strict mode
...
- Unknown `data-for` attributes that don't match any target IDs
- Target elements that have no corresponding `data-for` override
...
- `file` = relative to current file
- `virtual` = from `src/` root
- Apache SSI includes do not support content targeting
