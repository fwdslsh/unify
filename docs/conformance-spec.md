...
The exemption's stated limit: a site that never uses `--base-url` and declares `Sitemap: /sitemap.xml` is promising a file it will never have, and §23 says nothing about it. That is the deliberate trade — silence for the site that is right at its deploy address, over a block for the site that is wrong everywhere — and the second case is a judgement about intent, which §6.1 keeps out of the publish path.
