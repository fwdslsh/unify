...
Decoding follows [RFC 3986](https://www.rfc-editor.org/rfc/rfc3986)'s own division rather than a local rule: a percent-encoded **unreserved** character is equivalent to the character itself, so `%2E` is `.` and `%41` is `A` and both decode; a **reserved delimiter** left encoded is deliberately not a delimiter, so a segment that would decode to contain `/` or `\` is left encoded. `/a%2Fb.html` therefore names one segment whose name contains a slash — something no filesystem holds — and matches nothing, which is the right answer. Decoding it into a separator would silently resolve it to `a/b.html`, a different file, and under `--pretty-urls` would rewrite the author's link to an address naming that different file. A malformed escape leaves its segment as written rather than failing the build with a parse error.
