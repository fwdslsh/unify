...
**Product-spec §6.3.9 and §4 are amended to match, in the release that ships this rewrite.** As shipped at the time of this section's rewrite, product-spec §6.3.9's own first sentence still promises the finding this subsection removes ("`tags` and `categories` carry an audit finding stating that they create neither collections nor taxonomies"), and §4's frontmatter paragraph repeats it ("`tags` and `categories` still build nothing and `unify audit` says so once per page"). Both are stale prose describing a diagnostic that no longer exists, not a second normative source this section must satisfy — §6.3.9's OWN closing sentence, quoted above, is what license the removal, so the passage is amended against itself. The edit is tracked so it is not lost between batches: see `_notes/release-0.9.0-implementation-plan.md`'s B6 scope note.
