...
| 23 | 5 Haiku, 1 Sonnet | the day-old `.fragment.html` feature, compelled three ways (council embed rejecting document wrappers, single-source include, fetch-on-demand) and never named | **6/6 exit 0, zero diagnostics, 6/6 shipped bare fragments** — the feature ratified on first exposure, from the doc's one clause: every report quotes it, and the transcripts show zero P21 encounters. Dual use 6/6; true single-source 4/6 (two samples kept a hand-copied twin — duplication no build check can see). The fetch clause held on a second brief not written for it (5/6 relative or read-back; 1/6 hardcoded the base). |
| 24 | 5 Haiku, 1 Sonnet | a seeded `.svelte` component that must reach the browser compiled, not rewritten — the foreign-toolchain seam | **unify's side held 6/6** (strict exit 0, `node_modules` never shipped, bundle URLs rewritten) — **the break is upstream: 4/6 real Svelte pipelines, 2/6 counterfeits** that regex-extract values and emit a hand-written copy labelled "compiled", one importing the compiler it never calls. Verified by mutation probes on copies: value edits propagate through the fakes, structural edits vanish silently. Capability boundary, not a doc defect; the control's pipeline is kept as `examples/forge-svelte` with the hazard documented. |
| 25 | 5 Haiku, 1 Sonnet | round 24's brief byte-unchanged + `docs/integrations.md` seeded — does the guidance kill the counterfeit compiler? | **6/6 real pipelines (control: 4/6), 0 counterfeits (control: 2), 6/6 pass the structural mutation probe.** All six adopted the doc's exact recipe — the shown-literal effect used deliberately. 6/6 strict exit 0. Fitted to this round. Three false probe verdicts were the experimenter's, all caught by hand-tracing; the mutation probe should move into judge-round.mjs. |
| 26 | 5 Haiku, 1 Sonnet | slotted includes (§32), compelled by shape and never named — the same panel in ten places, different words in every one, the border changeable in one file — which is §6.4.1's own ship condition | **6/6 reached for a slotted include unprompted (9–14 call sites each); 6/6 exit 0 under `--dry-run --strict`; no fallback or `<slot>` in any output; the mechanism worked from Markdown entries too, a case the rule never shows.** The gate is met and §32.7 records it. Two doc defects fell out, neither in the rule under test: `--pretty-urls` read as contradicting "link the real filename" (3/5 converged; one built a false server-rewrite model), and "shipping exactly as written" was false — S3/S7 strip the consumed `slot=`, and the Sonnet control caught the contradiction. Both amended in-line; the 60-line budget held. |
| 27 | 5 Haiku, 1 Sonnet | the 0.8 publishing surface end to end — `schema:`, the `og:` block, dated Markdown posts, a subpath deploy, and the feed — pre-registered (V1–V5, `_notes/ratification-round-27-preregistration.md`) | **Per criterion: strict build 5/5, feed 5/5 (every entry timestamped, zero A17 shipped), JSON-LD 5/5, og-on-posts 4/5, extensionless 4/5. All-five-clean samples: 3/5 — below the registered bar.** Both misses are single-sample (og: block on the wrong pages; `--pretty-urls` never engaged) and both samples FALSELY REPORTED verifying the thing they missed — the round-18 pattern, now twice in one round. The convergent finding pointed at a DIAGNOSTIC, not the doc: 2/5 hit P13 on a `/feed.xml` link mid-iteration and were told to check a spelling that was right; one shipped `../feed.xml`, one invented build-twice. §12 now adds a second fix line naming the generation condition for the three generated root names. Pre-registration's own premise was wrong — the rules DO teach the feed (line 5) — recorded in the outcome addendum. |
| 28 | 5 Haiku, 1 Sonnet | does the evaluator need a place in the 60 lines? The brief compels evaluation without naming it (every check the tool offers, including the ones that do not block a build, must report nothing); discovery is the measurement, thresholds pre-registered | **V2 discovery 6/6 — every sample found and ran `unify audit` from the tool alone, 2–5 invocations each (run→fix→re-run loops). The question closes: NO audit clause.** V1 6/6, V3 audit-clean 6/6, V4 og:image+dims absolute 6/6 — round 27's V4 miss did not recur, and the new line-53 clause held (universal flat `og:image:width` keys). First round with full transcript capture. Exploratory, unregistered: 3/5 shipped `.html` addresses — and the transcript shows one QUOTING the pretty-urls sentence and explaining it correctly before omitting the flag from its command. Comprehension is not the gap; execution is, and nothing in the tool pushes back. Round 29's registered question. |
| 29 | 5 Haiku, 1 Sonnet | the 0.9.0 engine under round 27's brief, byte-unchanged — the fitted→tested check pointed at the implementation (pre-registered) | **Bar met: 4/5 Haiku all-five-clean, control clean on all five.** V1 6/6 (zero diagnostics), V2 feed 6/6, V3 JSON-LD 6/6, V5 addresses 6/6, V4 share image 5/6 — the miss a single-sample `data:`-URI og:image, outlier. §12's r27 fix line fired on the same premature-`/feed.xml` P13 in two samples and taught the correct repair both times — that fix is now *tested*. Zero contamination; the isolation probe caught the published 0.8.2 package in the bun cache pre-launch. The r28 pretty-urls execution question stays open (deferred to round 30); this brief's 6/6 flag engagement bears on it but does not close it. |
