...
**A green sweep is not a statement about the inventory.** The file defends the rules its rows name — twenty-six of them against an inventory of 220 — and nothing else. "Every mutation killed" means those anchors are pinned; reading it as "the spec is pinned" would be a new way to reach the same false confidence this check exists to remove.
...
One known hole, stated rather than left to be rediscovered: **`bun test` hangs instead of failing when a `src/**` module cannot parse.** Tier 0 says a hang is a failure, and this is the one place that is not yet true. Reproduction: append a syntax error to `src/core/urls.js` and run the suite — every file through `tests/conformance/publish-sync.test.js` fails fast and correctly, then `tests/conformance/watch-dev.test.js` stalls indefinitely (confirmed past 200s; the same file exits in 292ms in isolation). `freePort()` and `start().ready` are both bounded, so the unbounded wait is one of that file's un-timed `fetch` calls or its rebuild wait. The consequence is that a change stopping the CLI from starting stalls CI rather than failing it. `run-mutations.mjs` carries its own per-run timeout and is unaffected; the suite's own hole is open.
