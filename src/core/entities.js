/**
 * `entities.js` — character-reference resolution for §20's page manifest, and
 * for nothing else.
 *
 * Read the scope restriction first, because it is the important part. §3's
 * splice engine and every module it feeds treat source bytes as bytes: an
 * include, a slot fill, and a head merge move markup around without ever
 * interpreting it, which is what makes "all bytes outside an edited span are
 * preserved exactly" (S01) true. Decoding anywhere in that path would break
 * it. The manifest is different in kind — it is a *reading* of finished
 * output, describing what a person will see rather than what the file
 * contains — so it is the one place where `&amp;` must become `&`.
 *
 * §20.3 fixes the accepted set: the numeric forms, and the named references of
 * HTML 4.01's three entity sets (Latin-1, symbols/mathematical/Greek, and
 * special). That is a closed, citable list rather than one implementation's
 * habit, which matters because these values are a versioned machine contract:
 * the search projection's `title`/`description`/`text` are named in
 * product-spec §6.5.2, and a field whose decoding depends on who wrote the
 * decoder cannot be one. A reference outside the set, or malformed, is left
 * exactly as written — unrecognised markup is never silently deleted.
 */

/**
 * HTML 4.01's named character references, as `name codepoint` pairs. Written
 * as one string rather than an object literal purely for legibility at this
 * size; parsed once at module load.
 */
const TABLE = `
...
/** @type {Map<string, string>} entity name (case-sensitive, as HTML defines it) -> the character */
const NAMED = new Map();
{
  const tokens = TABLE.split(/\s+/).filter(Boolean);
  for (let i = 0; i < tokens.length; i += 2) {
    NAMED.set(tokens[i], String.fromCodePoint(Number(tokens[i + 1])));
  }
}

/**
 * Codepoints no character reference may name. A reference to one is left as
 * written rather than turned into a replacement character, following this
 * module's general rule: what is not recognised is not rewritten.
 */
function isValidCodePoint(n) {
  if (!Number.isInteger(n) || n < 1 || n > 0x10ffff) return false;
  if (n >= 0xd800 && n <= 0xdfff) return false; // lone surrogate
  return true;
}

/**
 * §20.3 — resolve character references in one string of emitted text.
 *
 * Everything unmatched passes through byte-for-byte, including a bare `&`,
 * an unterminated reference, and a name outside the accepted set. That is
 * deliberate: an author who wrote `AT&T` in their page meant `AT&T`, and a
 * decoder that guesses at incomplete markup produces text the page does not
 * contain — the exact failure this function exists to end, pointed the other
 * way.
 * @param {string} s
 * @returns {string}
 */
