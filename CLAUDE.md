...
- **One exception, and it exists to remove a fork rather than add one** (§33.2, GEN-11): the `--generate` spawn passes `--no-install` when `process.versions.bun` is set. Bun auto-installs an unresolvable import, so without it a generator with a missing dependency exited 0 under bun and 1 under node — the runtimes disagreeing about whether there was a build at all, which G12 cannot catch because it compares emitted bytes and here one side emits none. Bun exposes no environment equivalent (seven were measured, including `bunfig.toml` via `XDG_CONFIG_HOME`), so the flag has to be an argument. The guard is `process.versions.bun` rather than the executable's name, which is `unify-linux` on the compiled binary — the path that most needs it, since its promise is a machine with neither runtime installed and it was reaching the network anyway. Any further runtime branch needs the same standard: it must make the runtimes agree, and it must be measured on all three launch paths
...
- **`docs/getting-started.md`** — the human tutorial. **`docs/cli-reference.md`** — every command and flag. **`docs/integrations.md`** — the compile-to-asset pattern for Svelte and kin, every literal tested. **`docs/guides/eleventy-htmx.md`** — the Eleventy + htmx stack over the `--generate` seam, with the division of responsibilities and the measured watch behaviour. **`docs/guides/catalog-and-search.md`** — the blog-list-and-search recipe built from `catalog.json`/`search-corpus.json`, every literal tested.
