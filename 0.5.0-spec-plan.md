...
- `_layout.html` files provide folder-scoped layouts
- `src/_includes/` for shared partials/layouts  
- `src/_includes/default-layout.html` as optional fallback layout
...
export class FileClassifier {
  ...
  isLayout(filePath, sourceRoot) {
    // Files named _layout.html = layouts
  }
...
export class LayoutDiscovery {
  ...
  findLayoutForPage(pagePath, sourceRoot) {
    // 1. Look for _layout.html in page's directory
    // 2. Climb up to source root, checking each directory
    // 3. Check for src/_includes/default-layout.html fallback
